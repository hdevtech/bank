             ...
             // hdev_route::access('/', false, false, false, $urlka);
              ...
              if ($rasms_stc->access()) {
                if ($urlk == $urlka) {
                  ...
                  $view .='<a  href="'.$urlka.'" class="lift nav-link active'.$power.'"'.$gen_id.$id.' title="'.'RASMS'.'---' .self::hdevstr($name).$preff.'"">';
                }else{
                  $preff = "";
                  if (constant("APP_MASK") != "") {
                      $preff = " -- " .constant("APP_MASK");
                    }
                  $view .='<a  href="'.$urlka.'" class="text-nowrap lift nav-link'.$power.'"'.$gen_id.$id.' title="'.'RASMS'.'---' .self::hdevstr($name).$preff.'"">';
                }
              }else{
                $error ="no";
              }
            ...
            if (!empty($link)) {
              ...
              if ($rasms_stc->access()) {
                if ($urlk == $urlka) {
                  ...
                  $view .='<a href="'.$urlka.'" class="nav-link active liftk '.$power.'"'.$gen_id.'title="'.'RASMS'.' - ' .self::hdevstr($name).$preff.'">';
                }else{
                  $preff = "";
                  if (constant("APP_MASK") != "") {
                      $preff = " -- " .constant("APP_MASK");
                    }
                  $view .='<a href="'.$urlka.'" class="nav-link liftk '.$power.'"'.$gen_id.'title="'.'RASMS'.' - ' .self::hdevstr($name).$preff.'">';
                }
              }else{
                $error = "no";
              }
          ...
          if (in_array("y", $sumv)) {
              if (in_array("y", $urkmult)) {
                $view = '<li class="nav-item has-treeview menu-open"'.$gen_id1.'>
                        <a href="#" class="ind nav-link active"'.$gen_id2.'>';
              }else{
                $view = '<li class="nav-item has-treeview"'.$gen_id1.'>
                         <a href="#" class="ind nav-link"'.$gen_id2.'>';
              }
              ...
              for ($i=1; $i <= count($name)-1; $i++) {
                ...
                if ($rasms_stc->access()) {
                  ...
                  if ($urlk == $urlka) {
                    ...
                    $view .='<a href="'.$urlka.'" class="nav-link active liftk "'.$pk.$gen_id.'title="'.'RASMS'.' - ' .self::hdevstr(trim($name[$i])).$preff.'">';
                  }else{
                    $preff = "";
                    if (constant("APP_MASK") != "") {
                        $preff = " -- " .constant("APP_MASK");
                      }
                    $view .='<a href="'.$urlka.'" class="nav-link liftk "'.$pk.$gen_id.'title="'.'RASMS'.' - ' .self::hdevstr(trim($name[$i])).$preff.'">';
