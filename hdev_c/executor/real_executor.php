  ...
  class hdev_data
  {
    ...
    public static function locations($ret='province',$prov="",$dist="",$sect="",$cell="")
    {
      ...
      elseif ($ret == "province") {
        ...
        $ret = '<option value="">---choose province---</option>';
        ...
        $ret = '<option value="">---choose district---</option>';
        foreach ($v_ref as $retloc) {
        ...
        $ret = '<option value="">---choose sector---</option>';
        foreach ($v_ref as $retloc) {
        ...
        $ret = '<option value="">---choose cell---</option>';
        foreach ($v_ref as $retloc) {
        ...
        $ret = '<option value="">---choose village---</option>';
        //print_r($cell);
        foreach ($v_ref as $retloc) {
          //print_r($v_ref);
    ...
    public static function active_user($res="")
    {
      if (hdev_log::loged()) {
        ...
        switch ($funct) {
          ...
          break;
          case 'client':
            $ret = hdev_data::client($u,['data']);
            $resp['name'] = $ret['c_name'];
            $resp['username'] = $ret['c_email'];
            $resp['email'] = $ret['c_email'];
            $resp['fid'] = ucfirst($funct);
          break;
          case 'cashier':
            $ret = hdev_data::cashier($u,['data']);
            $resp['name'] = $ret['c_name'];
            $resp['username'] = $ret['c_email'];
            $resp['email'] = $ret['c_email'];
            $resp['fid'] = ucfirst($funct);
          break;    
          case 'loan_officer':
            $ret = hdev_data::loan_officer($u,['data']);
            $resp['name'] = $ret['lo_name'];
            $resp['username'] = $ret['lo_email'];
            $resp['email'] = $ret['lo_email'];
            $resp['fid'] = ucfirst($funct);
          break;                
          case 'provider':
    ...
    public static function client($l_id="",$param=[1])
    {
      $rt = new hdev_db();
      $tab = $rt->table("client");
      $v_ref = array();
      if (isset($param[0]) && $param[0] == 1) {
        ...
        $v_ref = $rt->select("SELECT count(*) as All_rec FROM $tab WHERE c_name !='' AND c_status = 2");
      ...
      elseif (isset($param[0]) && $param[0] == "exist") {
        $retur = false;
        $v_ref = $rt->select("SELECT * FROM $tab WHERE c_id = :l_id AND c_status=1",[[":l_id",$l_id]]);
      ...
      elseif (isset($param[0]) && $param[0] == "tel") {
        $retur = false;
        $v_ref = $rt->select("SELECT * FROM $tab WHERE c_tell = :c_tel AND c_status=1",[[":c_tel",$l_id]]);
        if (isset($v_ref[0]['c_id']) && !empty($v_ref[0]['c_id'])) {
          $retur = $v_ref;
        }else{
          $retur = false;
    ...
    public static function cashier($l_id="",$param=[1])
    {
      $rt = new hdev_db();
    ...
    public static function loan_officer($l_id="",$param=[1])
    {
      $rt = new hdev_db();
      $tab = $rt->table("loan_officer");
      $v_ref = array();
      if (isset($param[0]) && $param[0] == 1) {
        $v_ref = $rt->select("SELECT * FROM $tab WHERE lo_name !='' AND lo_status=1");
        return $v_ref;
      }elseif (isset($param[0]) && $param[0] == "all") {
        $v_ref = $rt->select("SELECT * FROM $tab WHERE lo_name !='' AND lo_status != 2");
        return $v_ref;
      }elseif (isset($param[0]) && $param[0] == "approve") {
        $v_ref = $rt->select("SELECT * FROM $tab WHERE lo_name !='' AND lo_status = 2");
        return $v_ref;
      }elseif (isset($param[0]) && $param[0] == "approve_count") {
        $v_ref = $rt->select("SELECT count(*) as All_rec FROM $tab WHERE lo_name !='' AND lo_status = 2");
        $ret = ($v_ref[0]["all_rec"] > 0) ? $v_ref[0]["all_rec"] : "" ;
        return $ret;
      }elseif (isset($param[0]) && $param[0] == "delete") {
        $v_ref = $rt->select("SELECT * FROM $tab WHERE lo_name !='' AND lo_status = 0");
        return $v_ref;
      }elseif (isset($param[0]) && $param[0] == "delete_count") {
        $v_ref = $rt->select("SELECT count(*) as All_rec FROM $tab WHERE lo_name !='' AND lo_status = 0");
        $ret = ($v_ref[0]["all_rec"] > 0) ? $v_ref[0]["all_rec"] : "" ;
        return $ret;
      }
      elseif (isset($param[0]) && $param[0] == "exist") {
        $retur = false;
        $v_ref = $rt->select("SELECT * FROM $tab WHERE lo_id = :l_id AND lo_status=1",[[":l_id",$l_id]]);
        if (isset($v_ref[0]['lo_id']) && !empty($v_ref[0]['lo_id'])) {
          $retur = "yes";
        }
        return $retur;
      }elseif (isset($param[0]) && $param[0] == "data") {
        $retur = false;
        $v_ref = $rt->select("SELECT * FROM $tab WHERE lo_id = :lo_id",[[":lo_id",$l_id]]);
        if (isset($v_ref[0]['lo_id']) && !empty($v_ref[0]['lo_id'])) {
          $retur = $v_ref[0];
        }
        return $retur;
        ...
        $retur = false;
        $v_ref = $rt->select("SELECT * FROM $tab WHERE lo_id = :lo_id AND c_status=1",[[":lo_id",$l_id]]);
        if (isset($v_ref[0]['lo_id']) && !empty($v_ref[0]['lo_id'])) {
          $retur = $v_ref[0];
        }else{
          $retur = false;
      ...
      elseif (isset($param[0]) && $param[0] == "tel") {
        $retur = false;
        $v_ref = $rt->select("SELECT * FROM $tab WHERE lo_tell = :lo_tel AND lo_status=1",[[":lo_tel",$l_id]]);
        if (isset($v_ref[0]['lo_id']) && !empty($v_ref[0]['lo_id'])) {
          $retur = $v_ref;
        }else{
          $retur = false;
    ...
    public static function withdraw($l_id="",$param=[1])
    {
      $rt = new hdev_db();
      $tab = $rt->table("withdraw");
      $v_ref = array();
      if (isset($param[0]) && $param[0] == 1) {
        $v_ref = $rt->select("SELECT * FROM $tab WHERE w_id !='' AND gen_status=1");
        return $v_ref;
      }elseif (isset($param[0]) && $param[0] == "all") {
        $v_ref = $rt->select("SELECT * FROM $tab WHERE w_id !='' AND gen_status != 2");
        return $v_ref;
      }elseif (isset($param[0]) && $param[0] == "approve") {
        $sq = "";
        if (hdev_log::fid() == "client") {
          $sq = "AND c_id=".hdev_log::uid();
        }
        $v_ref = $rt->select("SELECT * FROM $tab WHERE w_id !='' $sq AND r_approve_status = 1");
        return $v_ref;

      }elseif (isset($param[0]) && $param[0] == "wait") {
        $sq = "";
        if (hdev_log::fid() == "client") {
          $sq = "AND c_id=".hdev_log::uid();
        }
        $v_ref = $rt->select("SELECT * FROM $tab WHERE w_id !='' $sq AND r_approve_status IS NULL");
        return $v_ref;
        ...
        $v_ref = $rt->select("SELECT * FROM $tab WHERE w_id !='' $sq AND r_approve_status = 0");
        return $v_ref;
      }elseif (isset($param[0]) && $param[0] == "data") {
        $retur = false;
        $v_ref = $rt->select("SELECT * FROM $tab WHERE w_id = :w_id",[[":w_id",$l_id]]);
        if (isset($v_ref[0]['w_id']) && !empty($v_ref[0]['w_id'])) {
          $retur = $v_ref[0];
        }
        return $retur;
      }
    }   
    public static function loan($l_id="",$param=[1])
    {
      $rt = new hdev_db();
      $tab = $rt->table("loan");
      $v_ref = array();
      if (isset($param[0]) && $param[0] == 1) {
        $v_ref = $rt->select("SELECT * FROM $tab WHERE l_id !='' AND gen_status=1");
        return $v_ref;
      }elseif (isset($param[0]) && $param[0] == "all") {
        $v_ref = $rt->select("SELECT * FROM $tab WHERE l_id !='' AND gen_status != 2");
        return $v_ref;
      }elseif (isset($param[0]) && $param[0] == "approve") {
        $sq = "";
        if (hdev_log::fid() == "client") {
        ...
        $v_ref = $rt->select("SELECT * FROM $tab WHERE l_id !='' $sq AND lo_approve_status = 1");
        return $v_ref;
        ...
        $v_ref = $rt->select("SELECT * FROM $tab WHERE l_id !='' $sq AND lo_approve_status IS NULL");
        return $v_ref;
        ...
        $v_ref = $rt->select("SELECT * FROM $tab WHERE l_id !='' $sq AND lo_approve_status = 0");
        return $v_ref;
      }elseif (isset($param[0]) && $param[0] == "data") {
        $retur = false;
        $v_ref = $rt->select("SELECT * FROM $tab WHERE l_id = :l_id",[[":l_id",$l_id]]);
    ...
    public static function statement($s_id="",$param=[1])
    {
      $rt = new hdev_db();
      $tab = $rt->table("statement");
      $v_ref = array();
      if (isset($param[0]) && $param[0] == 1) {
        $v_ref = $rt->select("SELECT * FROM $tab WHERE s_id !='' AND gen_status=1");
        return $v_ref;
      }elseif (isset($param[0]) && $param[0] == "all") {
        $v_ref = $rt->select("SELECT * FROM $tab WHERE s_id !='' AND gen_status != 2");
        return $v_ref;
      }elseif (isset($param[0]) && $param[0] == "approve") {
        $sq = "";
        if (hdev_log::fid() == "client") {
        ...
        $v_ref = $rt->select("SELECT * FROM $tab WHERE s_id !='' $sq AND r_approve_status = 1");
        return $v_ref;
        ...
        $v_ref = $rt->select("SELECT * FROM $tab WHERE s_id !='' $sq AND r_approve_status IS NULL");
        return $v_ref;
        ...
        $v_ref = $rt->select("SELECT * FROM $tab WHERE s_id !='' $sq AND r_approve_status = 0");
        return $v_ref;
      }elseif (isset($param[0]) && $param[0] == "data") {
        $retur = false;
        $v_ref = $rt->select("SELECT * FROM $tab WHERE s_id = :s_id",[[":s_id",$s_id]]);
